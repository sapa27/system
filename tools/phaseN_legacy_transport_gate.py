...
PHASE5_TOTAL_SOURCE_BUDGET = 4865000
...
def main():
    ...
    ok('Vercel build runs Production current gate', 'tools/phaseN_legacy_transport_gate.py' in str(vercel.get('buildCommand','')), str(vercel.get('buildCommand','')))
    ok('package build runs Production current gate', 'phaseN_legacy_transport_gate.py' in str(package.get('scripts',{}).get('build','')), str(package.get('scripts',{}).get('build','')))
