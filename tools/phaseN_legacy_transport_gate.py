...
PHASE5_SIZE_BUDGETS = {
    ...
    "gas-backend/Scripts_Core_Runtime.html": 359000,
    "github-pages/partials/Scripts_Core_Runtime.html": 359000,
    "gas-backend/Scripts_Page_Meeting.html": 245500,
    "github-pages/partials/Scripts_Page_Meeting.html": 245700,
    "gas-backend/Code_30_Domain_Cases.gs": 327000,
    "gas-backend/Code_32_Domain_Budget.gs": 228000,
    "gas-backend/Code_00_PlatformCore.gs": 225500,
    "gas-backend/Scripts_Page_Budget.html": 161000,
    "github-pages/partials/Scripts_Page_Budget.html": 161100,
    "gas-backend/Scripts_Page_People.html": 159500,
    "github-pages/partials/Scripts_Page_People.html": 159700,
    "gas-backend/Index.html": 287500,
    "github-pages/index.html": 210500,
    "github-pages/critical-login-runtime.js": 87000,
    "github-pages/app-config.js": 16000,
    "github-pages/github-gas-transport.js": 38000,
}
PHASE5_TOTAL_SOURCE_BUDGET = 4930000
...
PHASE5_NEXT_SLIMMING_TARGETS = {
    ...
    "gas-backend/Code_00_PlatformCore.gs": 210000,
    "gas-backend/Scripts_Page_People.html": 140000,
}
...
def main():
    ...
    ok('pre-deploy QA script contract installed', package_scripts.get('qa:predeploy')=='npm run audit:strict && npm run build' and package_scripts.get('qa:strict')=='npm run audit:strict' and package_scripts.get('qa:vercel-build')=='npm run build', json.dumps({k:package_scripts.get(k) for k in ['qa:predeploy','qa:strict','qa:vercel-build']}, ensure_ascii=False))
    cache_ledger = manifest.get('cachePolicyMatrixLedger',{})
    cache_matrix_ok = all(token in app for token in ['cachePolicyMatrixOwner:"production-current-cache-policy-matrix"','clientApiCachePolicyMatrix:{','apiSearchCasesLite:{cacheable:!0,ttlSec:300,manualForceFresh:!0','apiSessionCheck:{cacheable:!1,ttlSec:0','apiBudgetGetSummary:{cacheable:!0,ttlSec:60','apiGetMasterDataBundle:{cacheable:!0,ttlSec:900'])
    transport_policy_ok = all(token in read('github-pages/github-gas-transport.js') for token in ['function cachePolicyFor(method)','policy.cacheable===!1','policy.ttlSec!=null?Number(policy.ttlSec)','cachePolicyMatrixOwner:cfg("cachePolicyMatrixOwner"'])
    ok('cache policy matrix installed and enforced', cache_ledger.get('stamp')=='production-current-cache-policy-matrix-2026-07-06-r1' and cache_ledger.get('owner')=='production-current-cache-policy-matrix' and cache_ledger.get('noApiContractChange') is True and cache_matrix_ok and transport_policy_ok, 'client cache policy matrix must govern TTL/cacheable/manual force-fresh without API or business logic change')
    ...
    core2=manifest.get('coreRuntimeRound2SlimmingLedger',{}) if isinstance(manifest.get('coreRuntimeRound2SlimmingLedger'), dict) else {}
    core_runtime_backend=read('gas-backend/Scripts_Core_Runtime.html')
    core_runtime_mirror=read('github-pages/partials/Scripts_Core_Runtime.html')
    ok('manifest records Core Runtime round 2 slimming', core2.get('stamp')=='production-current-core-runtime-round2-thin-facade-2026-07-06-r1' and core2.get('maxBackendCoreRuntimeBytes')==359000 and core2.get('maxMirrorCoreRuntimeBytes')==359000 and core2.get('noApiContractChange') is True and core2.get('businessLogicChanged') is False, 'TECH_DEBT_MANIFEST must record Core Runtime round 2 slimming without API or business logic change')
    ok('Core Runtime round 2 script metadata compacted', core_runtime_backend.count('<script id=')<=1 and 'app-production-runtime-contract-lock' in core_runtime_backend and core_runtime_mirror.count('<script id=')<=1 and len(core_runtime_backend.encode('utf-8'))<=359000 and len(core_runtime_mirror.encode('utf-8'))<=359000, 'Core Runtime should keep only required script id metadata and remain under 359 KB budgets')
    ...
    people_page=read('gas-backend/Scripts_Page_People.html')
    people_mirror=read('github-pages/partials/Scripts_Page_People.html')
    people_slim=manifest.get('peoplePageSlimmingLedger',{}) if isinstance(manifest.get('peoplePageSlimmingLedger'), dict) else {}
    ok('manifest records People page slimming cleanup', people_slim.get('stamp')=='production-current-people-page-helper-owner-slim-2026-07-06-r1' and people_slim.get('maxBackendPeopleBytes')==159500 and people_slim.get('maxMirrorPeopleBytes')==159700 and people_slim.get('noApiContractChange') is True and people_slim.get('businessLogicChanged') is False, 'TECH_DEBT_MANIFEST must record People page helper-owner slimming without API or business logic change')
    ok('People page helper owner installed', 'var $pa=Array.isArray,$po=Object.assign,$ph=Object.prototype.hasOwnProperty;' in people_page and 'var $pa=Array.isArray,$po=Object.assign,$ph=Object.prototype.hasOwnProperty;' in people_mirror and people_page.count('Array.isArray(')==0 and people_page.count('Object.assign(')==0 and people_page.count('Object.prototype.hasOwnProperty.call(')==0 and len(people_page.encode('utf-8'))<=159500 and len(people_mirror.encode('utf-8'))<=159700, 'People page should use $pa/$po/$ph helper owner and remain under new budgets')
    platform_slim=manifest.get('platformCoreSlimmingLedger',{}) if isinstance(manifest.get('platformCoreSlimmingLedger'), dict) else {}
    ok('manifest records PlatformCore slimming cleanup', platform_slim.get('stamp')=='production-current-platform-core-helper-owner-slim-2026-07-06-r1' and platform_slim.get('maxBackendPlatformCoreBytes')==225500 and platform_slim.get('noApiContractChange') is True and platform_slim.get('businessLogicChanged') is False, 'TECH_DEBT_MANIFEST must record PlatformCore helper-owner slimming without API or business logic change')
    ok('PlatformCore helper owner installed', 'function _00A_(v){return Array.isArray(v)}function _00O_(){return Object.assign.apply(Object,arguments)}function _00H_(o,k){return Object.prototype.hasOwnProperty.call(o,k)}' in platform and len(platform.encode('utf-8'))<=225500, 'PlatformCore must own low-level Array/Object/hasOwn helpers and remain under 225.5 KB')
    ...
    qa_ledger = manifest.get('preDeployQaContractLedger',{})
    ok('manifest records pre-deploy QA contract', isinstance(qa_ledger, dict) and qa_ledger.get('stamp')=='production-current-predeploy-qa-contract-2026-07-06-r1' and qa_ledger.get('strictGateMustPassBeforeDeploy') is True and qa_ledger.get('vercelBuildMustRemainHostSafe') is True and qa_ledger.get('noApiContractChange') is True and qa_ledger.get('businessLogicChanged') is False and isinstance(qa_ledger.get('manualSmokeChecklist'), list) and len(qa_ledger.get('manualSmokeChecklist'))>=10, 'TECH_DEBT_MANIFEST must record the mandatory QA contract before deploy')
    ok('manifest records search and meeting runtime bugfix', isinstance(manifest.get('searchMeetingRuntimeBugfixLedger'), dict) and manifest.get('searchMeetingRuntimeBugfixLedger',{}).get('stamp')=='production-current-search-meeting-runtime-bugfix-2026-07-06-r1' and manifest.get('searchMeetingRuntimeBugfixLedger',{}).get('noApiContractChange') is True and manifest.get('searchMeetingRuntimeBugfixLedger',{}).get('businessLogicChanged') is False, 'TECH_DEBT_MANIFEST must record search refresh and meeting agenda fallback bugfix without API/business logic change')
    ok('manifest records Budget UI modernization', isinstance(manifest.get('budgetUiModernizationLedger'), dict) and manifest.get('budgetUiModernizationLedger',{}).get('stamp')=='production-current-budget-ui-modern-current-2026-07-06-r1' and manifest.get('budgetUiModernizationLedger',{}).get('noApiContractChange') is True and manifest.get('budgetUiModernizationLedger',{}).get('businessLogicChanged') is False and manifest.get('budgetUiModernizationLedger',{}).get('maxBackendBudgetPageBytes')==161000 and manifest.get('budgetUiModernizationLedger',{}).get('maxMirrorBudgetPageBytes')==161100, 'TECH_DEBT_MANIFEST must record Budget UI modernization without API or business logic change')
    ok('manifest records all systems UI modernization', isinstance(manifest.get('allSystemsUiModernizationLedger'), dict) and manifest.get('allSystemsUiModernizationLedger',{}).get('stamp')=='production-current-all-systems-ui-modern-2026-07-06-r1' and manifest.get('allSystemsUiModernizationLedger',{}).get('owner')=='app-global-ui-modern-current' and manifest.get('allSystemsUiModernizationLedger',{}).get('noApiContractChange') is True and manifest.get('allSystemsUiModernizationLedger',{}).get('businessLogicChanged') is False and manifest.get('allSystemsUiModernizationLedger',{}).get('maxGasIndexBytes')==287500 and manifest.get('allSystemsUiModernizationLedger',{}).get('maxStaticIndexBytes')==210500, 'TECH_DEBT_MANIFEST must record all-systems UI modernization without API or business logic change')
    static_index = read('github-pages/index.html')
    ok('all systems UI owner installed in both indexes', all(token in gas_index and token in static_index for token in ['app-global-ui-modern-current','--ui-modern-blue','#p-search','#p-track','#p-report','#p-petitioner','#p-meeting','#p-committee-meeting','#p-personnel','#p-budget','#p-admin']) and len(gas_index.encode('utf-8')) <= 287500 and len(static_index.encode('utf-8')) <= 210500, 'global UI owner must style every major system page in GAS and static indexes and remain under size budget')
    ui_token_ledger=manifest.get('uiTokenSystemLedger',{}) if isinstance(manifest.get('uiTokenSystemLedger'), dict) else {}
    required_ui_tokens=['app-ui-token-system-current','--app-color-primary','--app-color-accent','--app-color-surface','--app-color-line','--app-radius-sm','--app-radius-md','--app-radius-lg','--app-radius-xl','--app-shadow-soft','--app-shadow-card','--app-touch-target','--app-space-page']
    ok('UI token system owner installed', ui_token_ledger.get('stamp')=='production-current-ui-token-system-2026-07-06-r1' and ui_token_ledger.get('owner')=='app-ui-token-system-current' and ui_token_ledger.get('noApiContractChange') is True and ui_token_ledger.get('businessLogicChanged') is False and all(t in gas_index and t in static_index for t in required_ui_tokens), 'Index.html must define shared app-ui-token-system-current tokens without API/business logic changes')
    meeting_page = read('gas-backend/Scripts_Page_Meeting.html')
    report_track_page = read('gas-backend/Scripts_Page_ReportTrack.html')
    ok('Search page manual search bypasses stale page cache', 'forceFreshNext=!0' in compact(report_track_page) and 'search-report-manual-force-fresh' in report_track_page and 'if(!manualFresh&&cachedEntry' in compact(report_track_page), 'manual Search action must bypass cached apiSearchCasesLite results and request fresh data')
    ok('Meeting agenda accordion native fallback installed', 'function toggleAgendaCollapse' in meeting_page and '#committee-agenda-accordion [data-bs-toggle="collapse"]' in meeting_page and 'toggleAgendaCollapse(btn)' in meeting_page, 'agenda 2/3/4 must open even when Bootstrap collapse runtime is unavailable')
    ok('committee meeting mobile form collapse installed', isinstance(manifest.get('committeeMeetingMobileUiLedger'), dict) and manifest.get('committeeMeetingMobileUiLedger',{}).get('stamp')=='production-current-committee-meeting-mobile-ui-collapse-2026-07-06-r1' and manifest.get('committeeMeetingMobileUiLedger',{}).get('owner')=='committee-meeting-mobile-ui-current' and manifest.get('committeeMeetingMobileUiLedger',{}).get('noApiContractChange') is True and 'committee-meeting-mobile-ui-current' in gas_index and 'committee-meeting-mobile-ui-current' in static_index and 'committee-meeting-form-toggle-btn' in gas_index and 'committee-meeting-form-body' in gas_index and 'function toggleMeetingFormCollapse' in meeting_page and '[data-committee-meeting-form-toggle]' in meeting_page, 'meeting form must support native collapse/expand and mobile UI owner without API/business logic change')
