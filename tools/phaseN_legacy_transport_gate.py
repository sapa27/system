...
PHASE5_TOTAL_SOURCE_BUDGET = 4837000
...
def main():
    ...
    ok('write route meta normalized from schema before backend boundary', 'function _routerEffectiveWriteMeta_' in router and 'out.write=!0,out.csrf=!0' in compact(router) and 'phase5WriteContractRouterReentry' in router and 'returnmeta?_routerEffectiveWriteMeta_(method,meta):null' in compact(router), 'write/csrf must be stamped before _backendBoundaryEnter_ so writeGateway sees write mode')
    ...
    ok('Thai datepicker fast-open guard installed', 'thai-datepicker-fast-open-current' in core_runtime and 'activeInput===input&&existing' in core_runtime and 'enhance(input),close()' in core_runtime and 'Date.now()-Number(lastOpenAt||0)<500' in core_runtime, 'avoid double open/rebuild from focusin+click and avoid parent-wide scan on input open')
    ...
    ok('Dashboard case status key canonical รับเรื่อง maps to s0', '"s1"' not in dash_status_body and 'รับเรื่อง|รับเข้า|^รับ$/.test(compact)?"s0"' in dash_status_body, 'รับเรื่อง/รับเข้า must count under เรื่องเข้าใหม่, not hidden s1')
