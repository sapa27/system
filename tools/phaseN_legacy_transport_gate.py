...
PHASE5_SIZE_BUDGETS = {
    ...
    "gas-backend/Scripts_Page_Meeting.html": 245000,
    "github-pages/partials/Scripts_Page_Meeting.html": 245100,
...
PHASE5_TOTAL_SOURCE_BUDGET = 4890000
...
def main():
    ...
    ok('manifest records meeting agenda add runtime fix', isinstance(manifest.get('meetingAgendaAddRuntimeFixLedger'), dict) and manifest.get('meetingAgendaAddRuntimeFixLedger',{}).get('stamp')=='production-current-meeting-agenda-add-runtime-fix-2026-07-06-r1' and manifest.get('meetingAgendaAddRuntimeFixLedger',{}).get('maxBackendMeetingBytes')==245000 and manifest.get('meetingAgendaAddRuntimeFixLedger',{}).get('noApiContractChange') is True and manifest.get('meetingAgendaAddRuntimeFixLedger',{}).get('businessLogicChanged') is False, 'TECH_DEBT_MANIFEST must record agenda-add runtime fallback without API/business logic change')
    ...
    ok('Meeting agenda add fallback installed', 'function handleAgendaAdd' in meeting_page and 'function bindAgendaAddFallback' in meeting_page and 'data-agenda-add' in meeting_page and 'ensureAgendaOpen(no)' in meeting_page and 'root.AppUi.enhance(row)' in meeting_page, 'agenda 2/3/4 add buttons must add rows through a direct fallback and open their pane')
