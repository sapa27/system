...
4. ตั้งค่า Vercel project จาก package root นี้ โดยใช้ค่าจาก `vercel.json`
   - Build Command: `python3 tools/generate_vercel_env.py && python3 tools/sync_frontend_partials.py --check && python3 tools/release_gate.py`
   - Output Directory: `github-pages`
5. Deploy
6. เปิด `/diagnostic.html` บน Vercel เพื่อตรวจ release stamp, public config, Vercel proxy และ Phase Q size gate
...
- คง Phase G security hardening: login POST only, no fast-login removed transport, require bridge ready message
- คง Phase K write schema unification
- ยังไม่เปิด Vercel proxy-only transport

## Transport policy ปัจจุบัน

- Public read เฉพาะ contract method ใน allow-list ใช้ removed transport
- Authenticated read ใช้ Vercel /api proxy และต้องรอ ready message จริง
- Write/mutation ใช้ Vercel /api proxy และต้องผ่าน write schema preflight
- Login ใช้ /api/login เท่านั้น
- ไม่ส่ง token/csrf ผ่าน removed transport URL
...
Phase N ควรเพิ่ม Vercel proxy-only transport เพื่อแทน iframe/removed transport transport ใน authenticated flow แต่ **Phase L ยังไม่เพิ่ม proxy** เพื่อให้การย้าย hosting ตรวจง่ายและย้อนกลับง่าย

## Phase N — Vercel proxy-only transport

Phase N routes login/read/write through same-origin Vercel API endpoints instead of iframe bridge/removed transport during normal Vercel deployment:
