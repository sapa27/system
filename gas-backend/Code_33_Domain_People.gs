...
PeopleDomain.dto = function(data, meta) {
  return Object.assign({
      domain: 'people',
      contractOwner: 'Code_33_Domain_People',
      stamp: PEOPLE_DOMAIN_DTO_REFACTOR_STAMP
    }, data || {
    }, {
      meta: Object.assign({
          dtoOnly: true
...
PeopleDomain.OWNER_CONTRACT = {
  ...
  domains: ["PeopleDomain",
    "PetitionerDomain",
    "AdminDomain"],
  sourceOfTruth: ["People",
    "Petitioners",
    "Users",
    "Subcommittees"],
...
function _peopleNormalizeKey_(key) {
  return _appIsFnName_("_normFieldKey_") ? _normFieldKey_(key): String(null == key ? "": key).replace(/[\u200B-\u200D\uFEFF]/g,
    "").replace(/[\s\n\r\t\-–—_()（）\[\]{}:：\/\\.]+/g, "").toLowerCase()
}
...
function _peopleCanonicalPersonnelDto_(row) {
  ...
  var name = String(row.name || row.fullName || row["ชื่อ-สกุล"] || "").trim(),
  position = String(row.position || row.role || row["ตำแหน่ง"] || "").trim(),
  status = String(row.status || row["สถานะ"] || "ดำรงตำแหน่ง").trim() || "ดำรงตำแหน่ง",
  fyKey = String(row.fy || row.fiscalYear || row["ปีงบประมาณ"] || "").replace(/[^0-9]/g, ""),
  salary = row.salaryAmount;
  return null != salary && "" !== salary ||(salary = row.monthlySalary),
  null != salary && "" !== salary ||(salary = row.salary),
  row.name = name,
  row.fullName = row.fullName || name,
  row.position = position,
  row.role = row.role || position,
  row.status = status,
  row.fy = row.fy || fyKey,
  row.fiscalYear = row.fiscalYear || fyKey,
  row.salaryAmount = null == salary ? "": salary,
  row.salary = row.salary || row.salaryAmount,
  row.monthlySalary = row.monthlySalary || row.salaryAmount,
  row.nameKey = _peopleNormalizeKey_(name),
  row.roleKey = _peopleNormalizeKey_(position),
  row.statusKey = _peopleNormalizeKey_(status),
  row.fyKey = fyKey,
  row.searchKey = _peopleNormalizedSearchKey_([row.name,
      row.position,
      row.personnelType,
      row.phone,
      row.proposedBy,
      row.status,
      row.fyKey]),
  row.isActive = _peopleIsActiveStatus_(status),
  row.schemaStamp = row.schemaStamp || "domain-schema-people-current",
  row.contractStamp = row.contractStamp || "people-personnel-dto-current",
  row
}
...
function _peopleInferFiscalYearFromDates_(startDate, endDate) {
  function parse(v) {
    ...
    var raw = String(normalized || "").trim(),
    m = raw.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
    if (m) {
      var iy = Number(m[1]);
      return iy > 2400 &&(iy -= 543),
      new Date(iy, Number(m[2]) - 1, Number(m[3]))
    }
    var d = new Date(raw);
...
function _peopleNormalizePersonnelSavePatch_(patch) {
  (patch = Object.assign({
      ...
      })).status = String(patch.status || "ดำรงตำแหน่ง").trim(),
  patch.startDate = _normalizePeopleDateValue_(patch.startDate || ""),
  patch.endDate = _peopleEndDateForStatus_(patch.status, patch.endDate || "");
  var fy = _peopleInferFiscalYearFromDates_(patch.startDate, patch.endDate) || "";
  return fy &&(patch.fy = patch.fy || fy, patch.fiscalYear = patch.fiscalYear || fy),
  patch.contractStamp = "people-save-backend-owned-current",
  patch
}
function _peopleFindActiveDuplicate_(sheetName, patch, currentId, mapper) {
  var name = _peopleNormalizeKey_((patch = patch || {
      }).name || ""),
  pos = _peopleNormalizeKey_(patch.position || "");
...
function _peopleActiveDuplicateResult_(dup) {
  ...
  return out.errorCode = "PEOPLE_ACTIVE_DUPLICATE",
  out.duplicate = dup || {
  },
  out
}
function normalizePersonnelStaffRow_(r) {
  var startDate = _peopleFirstValue_(r = r || {
      ...
      "วันเริ่ม-สิ้นสุด",
      "วันเริ่ม - สิ้นสุด"]) || "",
  endDate = _peopleFirstValue_(r, ["endDate",
      "วันที่สิ้นสุด",
      "วันสิ้นสุด",
      "สิ้นสุด",
      "วันสิ้นสุดตำแหน่ง",
  ...
  if (startDate && !endDate && /\s(?:-|–|—|ถึง)\s/.test(String(startDate))) {
    ...
    startDate = rangeParts[0] || startDate,
    endDate = rangeParts.slice(1).join(" ").trim()
  }
  startDate = _normalizePeopleDateValue_(startDate || ""),
  endDate = _normalizePeopleDateValue_(endDate || "");
  var fy = String(_peopleFirstValue_(r, ["fy",
        "fiscalYear",
        "FiscalYear",
        "ปีงบประมาณ",
        "ปีงบ",
        "ปีงบฯ",
        "ปีงบประมาณ (ที่ปรึกษา/บุคลากร)",
        "ปีงบประมาณที่ปรึกษาบุคลากร",
        "ปี"]) || "").replace(/[^0-9]/g, ""),
  status = String(_peopleFirstValue_(r, ["status",
        "สถานะ",
        "สถานะการดำรงตำแหน่ง",
        "Status"]) || "ดำรงตำแหน่ง").trim();
  return endDate = _peopleEndDateForStatus_(status, endDate),
  fy ||(fy = _peopleInferFiscalYearFromDates_(startDate, endDate)),
  _peopleCanonicalPersonnelDto_({
      id: String(_peopleFirstValue_(r, ["id",
            "รหัส",
            "ลำดับ",
            "เลขที่",
            "รหัสบุคลากร"]) || ""),
      fy: fy,
      name: String(_peopleFirstValue_(r, ["name",
            "ชื่อ-สกุล",
            "ชื่อสกุล",
            "ชื่อ - สกุล",
            "ชื่อ-นามสกุล",
            ...
            "fullName",
            "Name"]) || ""),
      position: String(_peopleFirstValue_(r, ["position",
            "ตำแหน่ง",
            "ตำแหน่งในคณะ",
            "role",
            "ตำแหน่งงาน",
            "Position"]) || ""),
      personnelType: String(_peopleFirstValue_(r, ["personnelType",
            "ประเภทบุคลากร",
            "ประเภท",
            "สถานภาพ"]) || ""),
      phone: _appPhoneForDisplay_(_peopleFirstValue_(r, ["phone",
            "เบอร์โทร",
            "เบอร์โทรศัพท์",
            "โทรศัพท์",
            "หมายเลขโทรศัพท์"]) || ""),
      proposedBy: String(_peopleFirstValue_(r, ["proposedBy",
            "กรรมาธิการผู้เสนอ",
            "ผู้เสนอ",
            "ผู้เสนอชื่อ"]) || ""),
      startDate: startDate,
      endDate: endDate,
      status: status,
      orderRef: String(_peopleFirstValue_(r, ["orderRef",
            "เลขที่คำสั่ง",
            "คำสั่ง",
            "คำสั่งแต่งตั้ง"]) || ""),
      remark: String(_peopleFirstValue_(r, ["remark",
            "หมายเหตุ",
            "รายละเอียด"]) || ""),
      createdAt: _peopleFirstValue_(r, ["createdAt",
          "วันที่สร้าง"]) || "",
      updatedAt: _peopleFirstValue_(r, ["updatedAt",
          "วันที่แก้ไข"]) || "",
      isGov: String(_peopleFirstValue_(r, ["isGov",
            "ข้าราชการ",
            "เป็นข้าราชการ",
            "สถานะข้าราชการ"]) || ""),
      commandRef: String(_peopleFirstValue_(r, ["commandRef",
            "commandFileUrl",
            "ไฟล์คำสั่ง",
            "คำสั่ง(แนบ)"]) || ""),
      commandFileUrl: String(_peopleFirstValue_(r, ["commandFileUrl",
            "commandRef",
            "ไฟล์คำสั่ง",
            "คำสั่ง(แนบ)"]) || ""),
      salaryAmount: _peopleFirstValue_(r, ["salaryAmount",
          "monthlySalary",
          "salary",
          "salaryBaht",
          "เงินเดือน (บาท)",
          ...
          "เงินเดือนบาท",
          "amount"]) || "",
      salary: _peopleFirstValue_(r, ["salary",
          "salaryAmount",
          "monthlySalary",
          "salaryBaht",
          "เงินเดือน (บาท)",
          ...
          "เงินเดือนบาท",
          "amount"]) || "",
      isDeleted: _peopleBoolDeleted_(_peopleFirstValue_(r, ["isDeleted",
            "ลบ",
            "สถานะลบ"]) || r.isDeleted),
      deletedAt: _peopleFirstValue_(r, ["deletedAt",
          "วันที่ลบ"]) || ""
    })
}
...
function _peopleDataServiceRows_(sheetName, fields, opts) {
  ...
  _appIsFnName_("_appDataServiceRead_") ? _appDataServiceRead_(String(sheetName || ""), fields, Object.assign({
        owner: "people.domain",
        ttl: 120,
        includeDeleted: !1,
        requireCanonical: !1
      }, opts)) || []: fields.length && "function" == typeof readSheetProjectedObjectsCached_ ? readSheetProjectedObjectsCached_(String(sheetName || ""),
    fields, {
      includeDeleted: !0 === opts.includeDeleted,
      requireCanonical: !0 === opts.requireCanonical,
      ttl: opts.ttl || 120
...
function _readPeopleSheetRowsDirect_(sheetName) {
  ...
  function visible(r) {
    ...
    var first = String(_peopleFirstValue_(r, ["id",
          "petId",
          "name",
          "ชื่อ-สกุล",
          "ชื่อสกุล",
          ...
          "ลำดับ",
          "เลขที่"]) || "").trim(),
    second = String(_peopleFirstValue_(r, ["position",
          "ตำแหน่ง",
          "ตำแหน่งในคณะ",
          "role",
          "ตำแหน่งงาน",
          ...
          "เบอร์โทรศัพท์",
          "โทรศัพท์"]) || "").trim(),
    third = String(_peopleFirstValue_(r, ["fy",
          "fiscalYear",
          "FiscalYear",
          "ปีงบประมาณ",
          "ปีงบ",
  ...
  try {
    ...
    attempts.push("projected:" + String(projectedErr && projectedErr.message || projectedErr)),
    _appIsFnName_("_recordWarning_") && _recordWarning_("people.read.projected.failed", projectedErr,
      {
        sheetName: sheetName
      })
  }
  ...
  try {
    ...
    if (allowMatrixRead) {
      ...
      if (sh) {
        var matrix,
        direct = filterRows(rowsFromMatrix_("function" == typeof getSheetMatrixCached_ ? getSheetMatrixCached_(sh,
              0, {
    ...
    attempts.push("matrix:" + String(matrixErr && matrixErr.message || matrixErr)),
    _appIsFnName_("_recordWarning_") && _recordWarning_("people.read.matrix.failed", matrixErr, {
        sheetName: sheetName
      })
  }
  try {
...
function _normalizePeopleDateValue_(value) {
  ...
  if ("[object Date]" === Object.prototype.toString.call(value) && !isNaN(value.getTime()))return Utilities.formatDate(value,
    Session.getScriptTimeZone() || "Asia/Bangkok", "yyyy-MM-dd");
  ...
  if (/^\d{5,6}(?:\.\d+)?$/.test(raw)) {
    ...
    if (serial > 3e4 && serial < 7e4) {
      ...
      if (!isNaN(d0.getTime()))return Utilities.formatDate(d0, Session.getScriptTimeZone() || "Asia/Bangkok",
        "yyyy-MM-dd")
    }
    if (serial > 2e5 && serial < 3e5) {
      var d1 = new Date(Math.round(86400 *(serial - 25569) * 1e3));
      if (!isNaN(d1.getTime()))return d1 = new Date(d1.getFullYear() - 543, d1.getMonth(), d1.getDate()),
      Utilities.formatDate(d1, Session.getScriptTimeZone() || "Asia/Bangkok", "yyyy-MM-dd")
    }
  }
  ...
  if (iso) {
    var iy = Number(iso[1]);
    return iy > 2400 &&(iy -= 543),
    String(iy).padStart(4, "0") + "-" + String(Number(iso[2])).padStart(2, "0") + "-" + String(Number(iso[3])).padStart(2,
      "0")
  }
  ...
  if (dmy) {
    var y = Number(dmy[3]);
    return y > 2400 &&(y -= 543),
    String(y).padStart(4, "0") + "-" + String(Number(dmy[2])).padStart(2, "0") + "-" + String(Number(dmy[1])).padStart(2,
      "0")
  }
  var thaiMonths = {
    "มกราคม": 1,
  ...
  if (th && thaiMonths[th[2]]) {
    ...
    String(ty).padStart(4, "0") + "-" + String(thaiMonths[th[2]]).padStart(2, "0") + "-" + String(Number(th[1])).padStart(2,
      "0")
  }
  return raw
}
function _mapPersonnelBaseRow_(r) {
  var startDate = _normalizePeopleDateValue_(_peopleFirstValue_(r = r || {
        ...
        "วันเริ่มต้น",
        "วันเริ่ม-สิ้นสุด"]) || ""),
  endDate = _normalizePeopleDateValue_(_peopleFirstValue_(r, ["endDate",
        "วันที่สิ้นสุด",
        "วันสิ้นสุด",
        "สิ้นสุด",
        "วันสิ้นสุดตำแหน่ง"]) || ""),
  ...
  var fy = String(_peopleFirstValue_(r, ["fy",
        ...
        "ปีงบประมาณ (ที่ปรึกษา/บุคลากร)",
        "ปี"]) || "").replace(/[^0-9]/g, "");
  return fy ||(fy = _peopleInferFiscalYearFromDates_(startDate, endDate)),
  _peopleCanonicalPersonnelDto_({
      id: String(_peopleFirstValue_(r, ["id",
            "รหัส",
            "เลขที่",
            "รหัสบุคลากร"]) || ""),
      sortOrder: String(_peopleFirstValue_(r, ["sortOrder",
            "orderNo",
            "order",
            "displayOrder",
            "ลำดับ",
...
function _peopleDirectoryCacheStamp_() {
  var keys;
  return["personnel_comm",
    "personnel_staff",
    "personnel_op",
    "personnel_subcommittees"].map(function(k) {
      try {
...
function _peopleDirectoryIndex_(bundle) {
  ...
  var buckets = {
    comms: Array.isArray(bundle.comms) ? bundle.comms: [],
    staffs: Array.isArray(bundle.staffs) ? bundle.staffs: [],
    ops: Array.isArray(bundle.ops) ? bundle.ops: [],
    subcommitteeMembers: Array.isArray(bundle.subcommitteeMembers) ? bundle.subcommitteeMembers: []
  },
  byFy = {
  },
  byRole = {
  ...
  return Object.keys(buckets).forEach(function(name) {
      (buckets[name] || []).forEach(function(row) {
          ...
          total++; 
          var fy = String(row.fy || row.fiscalYear || row.fyKey || "").replace(/[^0-9]/g, "") || "ไม่ระบุ",
          role = String(row.position || row.role || row.personnelType || "ไม่ระบุ").trim() || "ไม่ระบุ",
          status = String(row.status || "ดำรงตำแหน่ง").trim() || "ดำรงตำแหน่ง"; 
          byFy[fy] = Number(byFy[fy] || 0) + 1,
          byRole[role] = Number(byRole[role] || 0) + 1,
          byStatus[status] = Number(byStatus[status] || 0) + 1,
          !0 !== row.isActive && "ดำรงตำแหน่ง" !== status || active++
        })
    }),
  {
    ok: !0,
    contract: "people-directory-index-current",
    totalRecords: total,
...
function _peopleSalaryPaymentsRows_() {
  try {
    return _readPeopleSheetRowsDirect_("SalaryPayments").map(function(row, idx) {
        ...
        var name = String(_peopleFirstValue_(row, ["personName",
              "name",
              "fullName",
              "ชื่อ-สกุล",
              "ชื่อ"]) || "").trim(),
        amount = Number(String(_peopleFirstValue_(row, ["monthlyRate",
                "amount",
                "salaryAmount",
                "salary",
                "เงินเดือน",
                "จำนวนเงิน"]) || 0).replace(/,/g, "")); 
        return {
          id: String(row.id || row.personId || "PAY-" +(idx + 1)),
          personId: String(row.personId || ""),
          name: name,
          personName: name,
          position: String(row.position || ""),
          salaryAmount: isFinite(amount) ? amount: 0,
          salary: isFinite(amount) ? amount: 0,
          monthlySalary: isFinite(amount) ? amount: 0,
          monthlyRate: isFinite(amount) ? amount: 0,
          fy: String(row.fy || row.fiscalYear || row["ปีงบประมาณ"] || ""),
          months: Number(row.months || 1) || 1,
          source: "SalaryPayments"
        }
      }).filter(function(row) {
...
function _Domain_getPersonnelDirectoryBundle() {
  ...
  try {
    if (_appIsFnName_("_AppCacheGetJson_")) {
      ...
      if (cachedDirectory && "object" == typeof cachedDirectory && Array.isArray(cachedDirectory.comms) && Array.isArray(cachedDirectory.staffs) && Array.isArray(cachedDirectory.ops)) {
        cachedDirectory.cacheHit = !0,
        cachedDirectory.cacheStatus = "directory-hit",
        cachedDirectory.directoryPerformance = !0,
        !cachedDirectory.directoryIndex && _appIsFnName_("_peopleDirectoryIndex_") &&(cachedDirectory.directoryIndex = _peopleDirectoryIndex_(cachedDirectory));
        try {
          _appIsFnName_("_requestScopePut_") &&(cachedDirectory = _requestScopePut_("peopleDirectoryBundle",
              bundleCacheKey, cachedDirectory))
        } 
        catch (_rsErr) {
  ...
  var bundle = {
    comms: readBucket("Personnel_Comm", _mapPersonnelBaseRow_, "C"),
    staffs: readBucket("Personnel_Staff", normalizePersonnelStaffRow_, "S"),
    ops: readBucket("Personnel_Op", _mapPersonnelBaseRow_, "OP"),
    subcommitteeMembers: readBucket("Personnel_Subcommittees", _mapPersonnelSubcommitteeRow_, "SCM"),
    cacheHit: !1,
    cacheStatus: "directory-miss",
    directoryPerformance: !0
  };
  bundle.directoryIndex = _appIsFnName_("_peopleDirectoryIndex_") ? _peopleDirectoryIndex_(bundle): null,
  ...
  try {
    _appIsFnName_("_requestScopePut_") &&(bundle = _requestScopePut_("peopleDirectoryBundle", bundleCacheKey,
        bundle))
  } 
...
function _peopleFindHeaderIndex_(headers, field) {
  for (var aliases = _peopleSaveHeaderAliases_()[field] || [field], normalized =(headers || []).map(_peopleNormHeader_),
    i = 0; 
    i < aliases.length; 
    i++) {
...
function _peopleFindExistingRowIndex_(data, headers, keyField, keyValue, patch) {
  var keyIdx = _peopleFindHeaderIndex_(headers, keyField || "id"),
  target = String(keyValue || "").trim();
  if (target && keyIdx > - 1)for (var i = 1; 
    i < data.length; 
    i++)if (String((data[i] || [])[keyIdx] || "").trim() === target)return i;
  var nameIdx = _peopleFindHeaderIndex_(headers, "name"),
  posIdx = _peopleFindHeaderIndex_(headers, "position"),
  startIdx = _peopleFindHeaderIndex_(headers, "startDate"),
  name = String(patch && patch.name || "").trim(),
  pos = String(patch && patch.position || "").trim(),
  start = String(patch && patch.startDate || "").trim();
  if (name && nameIdx > - 1)for (var r = 1; 
    r < data.length; 
    r++) {
    var row = data[r] || [],
    sameName = String(row[nameIdx] || "").trim() === name,
    samePos = posIdx < 0 || !pos || String(row[posIdx] || "").trim() === pos,
    sameStart = startIdx < 0 || !start || String(row[startIdx] || "").trim() === start;
    if (sameName && samePos && sameStart)return r
  }
  return - 1
}
function _peopleEnsureHeadersForDirectWrite_(sheetName, sh) {
  var schema = "object" == typeof CANONICAL_SHEETS_ && CANONICAL_SHEETS_[sheetName] ? CANONICAL_SHEETS_[sheetName].slice(): [],
  data = getSheetMatrix_(sh),
  headers = data.length ?(data[0] || []).map(function(h) {
...
function _peopleUpsertResilient_(sheetName, keyField, keyValue, patch, canonicalSaveFn) {
  ...
  headers = _peopleEnsureHeadersForDirectWrite_(sheetName, sh);
  if (!headers.length)throw new Error("ไม่พบหัวตารางของชีต " + sheetName);
  var data = getSheetMatrix_(sh);
  data.length ||(data = [headers]);
  var rowIndex = _peopleFindExistingRowIndex_(data, headers, keyField || "id", id, patch),
  now =(new Date).toISOString();
  patch.createdAt ||(patch.createdAt = rowIndex > 0 ? "": now),
  ...
  rowIndex > 0 ?(AppRepository.setRangeValues(sheetName, rowIndex + 1, 1, [row.slice(0, headers.length)],
      {
        ...
        mode: "update",
        directWrite: !0
      }, "บันทึกข้อมูลสำเร็จ")): (AppRepository.setRangeValues(sheetName, Math.max(sh.getLastRow(), 1) + 1,
      1, [row.slice(0, headers.length)], {
        invalidate: !1
      }), invalidateSheetCache_(sheetName), ok_({
        id: id || patch.id || "",
        mode: "create",
...
function _Domain_getPersonnelStaffs() {
  function normalizeRows(rawRows) {
    return(Array.isArray(rawRows) ? rawRows: []).map(function(row, idx) {
        ...
        row.startDate = _normalizePeopleDateValue_(row.startDate || ""),
        row.endDate = _normalizePeopleDateValue_(row.endDate || ""),
        row.status ||(row.status = "ดำรงตำแหน่ง"),
        row
      }).filter(function(row) {
...
function _Domain_savePersonnelComm(p) {
  return domainWrite_("savePersonnelComm", p, function(input) {
      var repo = _peopleRepo_("Personnel_Comm", "id"),
      id = String(input.id || "").trim() || "C-" + Date.now(),
      commStatus = String(input.status || "ดำรงตำแหน่ง").trim(),
      commEndDate = _peopleEndDateForStatus_(commStatus, input.endDate || ""),
      patch = {
        id: id,
        sortOrder: String(input.sortOrder || input.orderNo || input.order || "").trim(),
        name: String(input.name || "").trim(),
        position: String(input.position || "").trim(),
        phone: _appPhoneForSheet_(input.phone || ""),
      ...
      duplicate = _peopleFindActiveDuplicate_("Personnel_Comm", patch = _peopleNormalizePersonnelSavePatch_(patch),
        id, _mapPersonnelBaseRow_); 
      return duplicate ? _peopleActiveDuplicateResult_(duplicate): _peopleUpsertResilient_("Personnel_Comm",
        "id", id, patch, function() {
...
function _Domain_savePersonnelOp(p) {
  return domainWrite_("savePersonnelOp", p, function(input) {
      var repo = _peopleRepo_("Personnel_Op", "id"),
      id = String(input.id || "").trim() || "OP-" + Date.now(),
      opStatus = String(input.status || "ดำรงตำแหน่ง").trim(),
      opEndDate = _peopleEndDateForStatus_(opStatus, input.endDate || ""),
      patch = {
        id: id,
        name: String(input.name || "").trim(),
        position: String(input.position || "").trim(),
      ...
      duplicate = _peopleFindActiveDuplicate_("Personnel_Op", patch = _peopleNormalizePersonnelSavePatch_(patch),
        id, _mapPersonnelBaseRow_); 
      return duplicate ? _peopleActiveDuplicateResult_(duplicate): _peopleUpsertResilient_("Personnel_Op",
        "id", id, patch, function() {
          var result = repo.upsert(id, patch); 
          return ok_({
              id: result.key,
...
function _Domain_savePersonnelStaff(p) {
  return domainWrite_("savePersonnelStaff", p, function(input) {
      var repo = _peopleRepo_("Personnel_Staff", "id"),
      id = String(input.id || "").trim() || "S-" + Date.now(),
      staffStatus = String(input.status || "ดำรงตำแหน่ง").trim(),
      staffEndDate = _peopleEndDateForStatus_(staffStatus, input.endDate || ""),
      salaryValue = input.salaryAmount || input.monthlySalary || input.salary || input.salaryBaht || input["เงินเดือน (บาท)"] || input["เงินเดือน"] || "",
      patch = {
        id: id,
        name: String(input.name || "").trim(),
        position: String(input.position || input.otherPosition || "").trim(),
        personnelType: String(input.personnelType || "").trim(),
        phone: String(input.phone || "").trim(),
        proposedBy: String(input.proposedBy || "").trim(),
        startDate: input.startDate || "",
        endDate: staffEndDate,
        status: staffStatus,
        orderRef: String(input.orderRef || "").trim(),
        remark: String(input.remark || "").trim(),
        isGov: String(input.isGov || "").trim(),
        commandRef: String(input.commandRef || input.commandFileUrl || "").trim(),
        salaryAmount: salaryValue,
        salary: salaryValue,
        isDeleted: !1,
        deletedAt: ""
      },
      duplicate = _peopleFindActiveDuplicate_("Personnel_Staff", patch = _peopleNormalizePersonnelSavePatch_(patch),
        id, normalizePersonnelStaffRow_); 
      return duplicate ? _peopleActiveDuplicateResult_(duplicate): _peopleUpsertResilient_("Personnel_Staff",
        "id", id, patch, function() {
          var result = repo.upsert(id, patch); 
          return ok_({
              id: result.key,
...
function _subcommitteeDirectoryMap_() {
  ...
  try {
    (_Domain_getSubcommitteeList() || []).forEach(function(row) {
        ...
        id &&(byId[id] = row),
        name &&(byName[name.toLowerCase()] = row)
      })
  } 
  catch (e) {
...
function _mapPersonnelSubcommitteeRow_(r) {
  ...
  var name = String(_peopleFirstValue_(r, ["name",
        "ชื่อ-สกุล",
        "ชื่อสกุล",
        "fullName",
        "ชื่อ"]) || "").trim(),
  subcommitteeId = String(_peopleFirstValue_(r, ["subcommitteeId",
        "รหัสคณะอนุกรรมาธิการ"]) || "").trim(),
  subcommitteeName = String(_peopleFirstValue_(r, ["subcommitteeName",
        "subcommittee",
        "คณะอนุกรรมาธิการ",
        "ชื่อคณะอนุกรรมาธิการ"]) || "").trim(),
  directory = _subcommitteeDirectoryMap_();
  subcommitteeId && directory.byId[subcommitteeId] &&(subcommitteeName = subcommitteeName || String(directory.byId[subcommitteeId].name || "")),
  !subcommitteeId && subcommitteeName && directory.byName[subcommitteeName.toLowerCase()] &&(subcommitteeId = String(directory.byName[subcommitteeName.toLowerCase()].id || ""));
  var status = String(_peopleFirstValue_(r, ["status",
        "สถานะ"]) || "ดำรงตำแหน่ง").trim() || "ดำรงตำแหน่ง",
  startDate = _normalizePeopleDateValue_(_peopleFirstValue_(r, ["startDate",
        "วันเริ่ม",
        "วันที่เริ่ม",
        "วันเริ่ม-สิ้นสุด"]) || ""),
  endDate = _normalizePeopleDateValue_(_peopleFirstValue_(r, ["endDate",
        ...
        "วันที่สิ้นสุด"]) || "");
  endDate = _peopleEndDateForStatus_(status, endDate);
  var appointmentDate = _normalizePeopleDateValue_(_peopleFirstValue_(r, ["appointmentDate",
        "วันแต่งตั้ง",
        "วันที่แต่งตั้ง"]) || startDate || ""),
  fy = _peopleInferFiscalYearFromDates_(startDate || appointmentDate, endDate) || "";
  return _peopleCanonicalPersonnelDto_({
      ...
      sortOrder: String(_peopleFirstValue_(r, ["sortOrder",
            ...
            "ลำดับ",
            "ลำดับที่"]) || "").trim(),
      personId: String(_peopleFirstValue_(r, ["personId",
            "personnelId",
            "รหัสบุคคล"]) || ""),
      fy: fy,
      name: name,
      ...
      note: String(_peopleFirstValue_(r, ["note",
            "remark",
            "หมายเหตุ"]) || "").trim(),
      sourceRole: "subcommittee"
    })
}
function _Domain_getPersonnelSubcommittees() {
  try {
    ...
    return _appIsFnName_("_recordWarning_") && _recordWarning_("people.subcommitteeMembers.readRows",
      e),
    []
  }
}
function _Domain_savePersonnelSubcommittee(p) {
  return domainWrite_("savePersonnelSubcommittee", p, function(input) {
      ...
      var repo = _peopleRepo_("Personnel_Subcommittees", "id"),
      id = String(input.id || "").trim() || "SCM-" + Date.now(),
      name = String(input.name || input.fullName || "").trim(),
      subcommitteeName = String(input.subcommitteeName || input.subcommittee || "").trim(),
      subcommitteeId = String(input.subcommitteeId || "").trim(),
      directory = _subcommitteeDirectoryMap_(); 
      if (subcommitteeId && directory.byId[subcommitteeId] &&(subcommitteeName = subcommitteeName || String(directory.byId[subcommitteeId].name || "")),
        !subcommitteeId && subcommitteeName && directory.byName[subcommitteeName.toLowerCase()] &&(subcommitteeId = String(directory.byName[subcommitteeName.toLowerCase()].id || "")),
        !name)return err_("กรุณาระบุชื่อ-สกุล"); 
      if (!subcommitteeName)return err_("กรุณาเลือกคณะอนุกรรมาธิการ"); 
      var status = String(input.status || "ดำรงตำแหน่ง").trim() || "ดำรงตำแหน่ง",
      endDate = _peopleEndDateForStatus_(status, input.endDate || ""),
      patch = {
        id: id,
        sortOrder: String(input.sortOrder || input.orderNo || input.order || "").trim(),
        ...
        startDate: _normalizePeopleDateValue_(input.startDate || ""),
        endDate: _normalizePeopleDateValue_(endDate || ""),
        status: status,
        appointmentDate: _normalizePeopleDateValue_(input.appointmentDate || input.startDate || ""),
        note: String(input.note || input.remark || "").trim(),
        isDeleted: !1,
        deletedAt: ""
      },
...
function _subcommitteeDateValue_(value) {
  ...
  if (m) {
    var y = Number(m[3]);
    return y > 2400 &&(y -= 543),
    y < 2e3 ? null: new Date(y, Number(m[2]) - 1, Number(m[1]))
  }
  var iso = raw.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
  if (iso) {
    var yy = Number(iso[1]);
...
function _subcommitteeActiveDays_(startDate, endDate) {
  ...
  if (!start)return 0;
  var end = _subcommitteeDateValue_(endDate) || new Date,
  s = new Date(start.getFullYear(), start.getMonth(), start.getDate()),
  e = new Date(end.getFullYear(), end.getMonth(), end.getDate()),
  diff = Math.floor((e.getTime() - s.getTime()) / 864e5) + 1;
  return Math.max(0, diff || 0)
}
function _ensureSubcommitteeDateHeaders_() {
  try {
    var sh = getSheet_("Subcommittees"),
    schema = "undefined" != typeof SHEET_SCHEMAS && SHEET_SCHEMAS && Array.isArray(SHEET_SCHEMAS.Subcommittees) ? SHEET_SCHEMAS.Subcommittees.slice(): ["id",
      "name",
      "status",
      "appointmentDate",
      "dutyStartDate",
      ...
      "isDeleted",
      "deletedAt"],
    lastRow = Math.max(Number(sh.getLastRow && sh.getLastRow()) || 1, 1),
    lastCol = Math.max(Number(sh.getLastColumn && sh.getLastColumn()) || 0, schema.length, 1),
    values = AppRepository.getRangeValues("Subcommittees", 1, 1, lastRow, lastCol),
    ...
    var alias = {
      ...
      commencementdate: "dutyStartDate",
    ...
    return changed &&(AppRepository.setRangeValues("Subcommittees", 1, 1, out, {
          invalidate: !1
        }), lastCol > finalHeaders.length && AppRepository.clearRangeContent("Subcommittees", 1, finalHeaders.length + 1,
        Math.max(lastRow, 1), lastCol - finalHeaders.length), "function" == typeof invalidateSheetCache_ && invalidateSheetCache_("Subcommittees")),
    !0
  } 
  catch (e) {
    return _appIsFnName_("_recordWarning_") && _recordWarning_("people.subcommittee.ensureEnglishDateHeaders",
      e),
    !1
  }
}
function _subcommitteeStatusFromDates_(status, startDate, endDate) {
  return status = String(status || "").trim() || "ใช้งาน",
  endDate && "ใช้งาน" === status ? "สิ้นสุด": status
}
function _Domain_getSubcommitteeList() {
  ...
  function peopleSubcommitteePickCanonical_(row, keys) {
    return "undefined" != typeof AppBackendCore && AppBackendCore.pickNormalized ? AppBackendCore.pickNormalized(row,
      keys, ""): _peoplePick_(row, keys)
  }
  function toDto(row, idx) {
    row = row || {
    };
    var name = String(peopleSubcommitteePickCanonical_(row, ["name",
          "subcommitteeName",
          "committeeName",
          "title",
          "ชื่อคณะอนุกรรมาธิการ",
          ...
          "ชื่อ",
          "รายการ"]) || "").trim(),
    appointmentDate = String(peopleSubcommitteePickCanonical_(row, ["appointmentDate",
          "appointedDate",
          "วันที่แต่งตั้ง",
          "วันแต่งตั้ง"]) || "").trim(),
    startDate = String(peopleSubcommitteePickCanonical_(row, ["dutyStartDate",
          "startDate",
          "startWorkingDate",
          "effectiveDate",
          "วันเริ่มทำหน้าที่",
          ...
          "วันที่เริ่ม",
          "วันเริ่ม"]) || "").trim(),
    endDate = String(peopleSubcommitteePickCanonical_(row, ["endDate",
          "วันที่สิ้นสุด",
          "วันสิ้นสุด",
          "วันที่ยุติ",
          "วันยุติ"]) || "").trim(),
    status = _subcommitteeStatusFromDates_(String(peopleSubcommitteePickCanonical_(row, ["status",
            "active",
            "สถานะ",
            "activeStatus"]) || "ใช้งาน").trim() || "ใช้งาน", startDate, endDate),
    activeDays = _subcommitteeActiveDays_(startDate, endDate);
    ...
    startDate ||(activeDays = Number(peopleSubcommitteePickCanonical_(row, ["activeDays",
            "จำนวนวัน",
            "รวมวัน"]) || 0) || 0);
    var dueDays = cleanSubDays(peopleSubcommitteePickCanonical_(row, ["dueDays",
          "deadlineDays",
          "จำนวนวันครบกำหนด",
          "วันครบกำหนด(จำนวนวัน)"])),
    dueDate = String(peopleSubcommitteePickCanonical_(row, ["dueDate",
          "deadline",
          "วันครบกำหนด",
          "วันที่ครบกำหนด"]) || "").trim();
    /1900|GMT|เวลา|T\d{2}:\d{2}/i.test(dueDate) &&(dueDate = ""),
    startDate && dueDays &&(dueDate = _subcommitteeAddDays_(startDate, dueDays) || dueDate);
    var totalExtensionDays = String(peopleSubcommitteePickCanonical_(row, ["totalExtensionDays",
          "totalExtendDays",
          "รวมวันขยาย"]) || "").trim(),
    extendedDueDate = String(peopleSubcommitteePickCanonical_(row, ["extendedDueDate",
          "finalDueDate",
          "latestDueDate",
          "วันครบกำหนดหลังขยาย",
          "วันที่ครบกำหนดหลังขยาย"]) || "").trim() || dueDate;
    return {
      id: String(peopleSubcommitteePickCanonical_(row, ["id",
            "subcommitteeId",
            "SubcommitteeId",
            "รหัส",
            "ลำดับ",
            "เลขที่"]) || "SUB-" + String(idx + 1)).trim(),
      name: name,
      subcommitteeName: name,
      status: status,
      appointmentDate: appointmentDate,
      startDate: startDate,
      ...
      dueDays: dueDays,
      dueDate: dueDate,
      extensionDays: String(peopleSubcommitteePickCanonical_(row, ["extensionDays",
            "extendDays",
            "ขยายระยะเวลา",
            "จำนวนวันขยาย"]) || "").trim(),
      totalExtensionDays: totalExtensionDays,
      extendedDueDate: extendedDueDate,
      extensionHistory: String(peopleSubcommitteePickCanonical_(row, ["extensionHistory",
            "ประวัติการขยายเวลา"]) || "").trim(),
      remark: String(peopleSubcommitteePickCanonical_(row, ["remark",
            "note",
            "หมายเหตุ",
            "รายละเอียด"]) || "").trim(),
      updatedAt: String(peopleSubcommitteePickCanonical_(row, ["updatedAt",
            "createdAt",
            "วันที่แก้ไข",
            "วันที่สร้าง"]) || "").trim()
    }
  ...
  function readSheet(sheetName) {
    ...
    if (!allowFull)return[];
    var ss = "function" == typeof getSpreadsheet_ ? getSpreadsheet_(): null,
    sh = ss && _appIsFn_(ss.getSheetByName) ? ss.getSheetByName(sheetName): null;
    if (!sh)return[];
    ...
    var schema = "undefined" != typeof SHEET_SCHEMAS && SHEET_SCHEMAS && Array.isArray(SHEET_SCHEMAS.Subcommittees) ? SHEET_SCHEMAS.Subcommittees: [],
    aliases = "undefined" != typeof SHEET_HEADER_ALIASES && SHEET_HEADER_ALIASES && SHEET_HEADER_ALIASES.Subcommittees ? SHEET_HEADER_ALIASES.Subcommittees: {
    },
    rows = "undefined" != typeof AppBackendCore && AppBackendCore.matrixToObjects ? AppBackendCore.matrixToObjects(values,
      {
        ...
        includeDeleted: !1,
        headerResolver: function(raw, nk, i, aliasMap, _appAliases, schemaList) {
          return aliasMap && aliasMap[nk] || raw || schemaList[i] || "col" +(i + 1)
        }
      }): [];
    return _appIsFnName_("_requestScopeNoteRowsRead_") && _requestScopeNoteRowsRead_(sheetName, rows.length),
    rows
  }
  var raw = [];
  if (["Subcommittees",
      "Subcommittee",
      "คณะอนุกรรมาธิการ"].some(function(sheetName) {
        try {
          return(raw = readSheet(sheetName)).length > 0
        } 
        catch (e) {
          return _appIsFnName_("_recordWarning_") && _recordWarning_("people.subcommittee.sheet." + sheetName,
            e),
          !1
        }
      }), !raw.length)try {
...
function _subcommitteeAddDays_(baseDate, days) {
  var d = _subcommitteeDateValue_(baseDate),
  n = Number(days || 0) || 0;
  if (!d || !n)return "";
  var out = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  return out.setDate(out.getDate() + n),
  _subcommitteeFormatThaiDate_(out)
}
...
function _subcommitteeDeadlinePatch_(input, previous) {
  ...
  var startDate = String(input.dutyStartDate || input.startDate || input.startWorkingDate || input.effectiveDate || previous.dutyStartDate || previous.startDate || previous.startWorkingDate || "").trim(),
  dueDays = cleanDays(input.dueDays || input.deadlineDays || input["จำนวนวันครบกำหนด"] || previous.dueDays || ""),
  dueDate = String(input.dueDate || input.deadline || "").trim();
  /1900|GMT|เวลา|T\d{2}:\d{2}/i.test(dueDate) &&(dueDate = ""),
  startDate && dueDays &&(dueDate = _subcommitteeAddDays_(startDate, dueDays) || dueDate),
  dueDate ||(dueDate = String(previous.dueDate || previous.deadline || "").trim());
  var history = _subcommitteeExtensionHistory_(previous.extensionHistory || ""),
  newExtendDays = Number(cleanDays(input.extensionDays || input.extendDays || 0)) || 0,
  totalExtensionDays = Number(cleanDays(previous.totalExtensionDays || 0)) || 0,
  latestDue = String(previous.extendedDueDate || previous.finalDueDate || previous.latestDueDate || dueDate || "").trim();
  if (/1900|GMT|เวลา|T\d{2}:\d{2}/i.test(latestDue) &&(latestDue = ""), latestDue ||(latestDue = dueDate),
    newExtendDays > 0) {
    var base = latestDue || dueDate || startDate;
    latestDue = _subcommitteeAddDays_(base, newExtendDays) || latestDue || dueDate,
    totalExtensionDays += newExtendDays,
    history.push({
        days: newExtendDays,
        baseDate: base || "",
        extendedDueDate: latestDue || "",
  ...
  return {
    dueDays: dueDays,
    dueDate: dueDate,
    extensionDays: newExtendDays > 0 ? String(newExtendDays): "",
    totalExtensionDays: totalExtensionDays ? String(totalExtensionDays): "",
    extendedDueDate: latestDue || dueDate || "",
    extensionHistory: history.length ? JSON.stringify(history): String(previous.extensionHistory || "")
  }
}
function _Domain_saveSubcommittee(p) {
  return domainWrite_("saveSubcommittee", p, function(input) {
      _ensureSubcommitteeDateHeaders_(); 
      var repo = _peopleRepo_("Subcommittees", "id"),
      ...
      var resolvedId = id || "SUB-" + Date.now(),
      previous = {
      }; 
      try {
      ...
      var appointmentDate = String(input.appointmentDate || input.appointedDate || input["วันแต่งตั้ง"] || input["วันที่แต่งตั้ง"] || "").trim(),
      startDate = String(input.dutyStartDate || input.startDate || input.startWorkingDate || input.effectiveDate || "").trim(),
      endDate = String(input.endDate || "").trim(),
      status = _subcommitteeStatusFromDates_(String(input.status || "ใช้งาน"), startDate, endDate),
      deadlinePatch = _subcommitteeDeadlinePatch_(Object.assign({
          }, input, {
            startDate: startDate
          }), previous),
      result = repo.upsert(resolvedId, Object.assign({
            id: resolvedId,
            name: name,
            status: status,
            appointmentDate: appointmentDate,
            ...
            startDate: startDate,
            startWorkingDate: startDate,
            "วันเริ่มทำหน้าที่": startDate,
            "วันที่เริ่มทำหน้าที่": startDate,
            endDate: endDate,
            activeDays: _subcommitteeActiveDays_(startDate, endDate),
  ...
  try {
    ...
    function add_(row) {
      ...
      name && active_(row) && merged.push(Object.assign({
            ...
            id: text_(row.subcommitteeId || row.id || row.key || row.code || name) || name,
            subcommitteeId: text_(row.subcommitteeId || row.id || row.key || row.code || name) || name,
            name: name,
            subcommitteeName: name
          }))
    }
...
function _peopleParseThaiAddress_(value) {
  ...
  return[zip && zip[0],
    province && province[0],
    district && district[0],
    subDistrict && subDistrict[0]].forEach(function(token) {
      token &&(line = line.replace(token, " "))
    }),
  out.addressLine = line.replace(/\s+/g, " ").trim().replace(/[,]+$/, ""),
  out
}
function _peopleComposeThaiAddress_(parts) {
  parts = parts || {
  };
  var line = String(parts.addressLine || "").trim(),
  subDistrict = String(parts.subDistrict || "").trim(),
  district = String(parts.district || "").trim(),
  province = String(parts.province || "").trim(),
  postalCode = String(parts.postalCode || "").trim(),
  out = [];
  return line && out.push(line),
  subDistrict && out.push(("กรุงเทพมหานคร" === province ? "แขวง": "ต.") + subDistrict),
  district && out.push(("กรุงเทพมหานคร" === province ? "เขต": "อ.") + district),
  province && out.push("จ." + province),
  postalCode && out.push(postalCode),
  out.join(" ")
}
function _peopleCanonicalPetitionerAddress_(r) {
  r = r || {
  };
  var sourceAddress = String(_peopleFirstValue_(r, ["address",
        "fullAddress",
        "ที่อยู่"]) || "").trim(),
  parsed = _peopleParseThaiAddress_(sourceAddress),
  directLine = String(_peopleFirstValue_(r, ["addressLine",
        "location",
        "สถานที่",
        "บ้านเลขที่ / ถนน / หมู่บ้าน",
        "บ้านเลขที่ถนนหมู่บ้าน"]) || "").trim(),
  directSubDistrict = String(_peopleFirstValue_(r, ["subDistrict",
        "subdistrict",
        "tambon",
        "ตำบล",
        "แขวง"]) || "").trim(),
  directDistrict = String(_peopleFirstValue_(r, ["district",
        "amphoe",
        "amphur",
        "อำเภอ",
        "เขต"]) || "").trim(),
  directProvince = String(_peopleFirstValue_(r, ["province",
        "จังหวัด"]) || "").trim(),
  directPostalCode = String(_peopleFirstValue_(r, ["postalCode",
        "postcode",
        "zipCode",
        "zip",
        "รหัสไปรษณีย์"]) || "").trim(),
  looksLikeProjectedAddressLine;
  ...
  return parts.address = _peopleComposeThaiAddress_(parts) || sourceAddress,
  parts
}
function _peopleEnsurePetitionerAddressColumns_() {
  ["addressLine",
    ...
    "province",
    "postalCode",
    "address"].forEach(function(header) {
      try {
...
function normalizePetitionerRow_(r) {
  ...
  return {
    petId: String(_peopleFirstValue_(r, ["petId",
          "id",
          "petitionerId"]) || ""),
    name: String(_peopleFirstValue_(r, ["name",
          "fullName",
    ...
    petitionerId: String(_peopleFirstValue_(r, ["petitionerId",
          "เลขประจำตัวผู้ร้อง"]) || "").trim(),
    idCard: String(_peopleFirstValue_(r, ["idCard",
          "citizenId",
          "nationalId",
          "เลขบัตรประชาชน"]) || "").trim(),
    caseId: String(_peopleFirstValue_(r, ["caseId",
          "เลขเรื่อง"]) || "").trim(),
    remark: String(_peopleFirstValue_(r, ["remark",
          "note",
          "หมายเหตุ"]) || "").trim(),
    createdAt: String(_peopleFirstValue_(r, ["createdAt",
          "วันที่สร้าง"]) || ""),
    updatedAt: String(_peopleFirstValue_(r, ["updatedAt",
          "วันที่แก้ไข"]) || ""),
    isDeleted: _peopleBoolDeleted_(_peopleFirstValue_(r, ["isDeleted",
          "ลบ",
          "สถานะลบ"]) || r.isDeleted),
    deletedAt: String(_peopleFirstValue_(r, ["deletedAt",
          "วันที่ลบ"]) || "")
  }
}
function _Domain_getPetitioners() {
  ...
  try {
    var main = _appIsFnName_("_listMainDataRows_") ? _listMainDataRows_(!1): [],
    map = {
    };
    return(Array.isArray(main) ? main: []).forEach(function(row, idx) {
        row = row || {
        }; 
        var name = String(_peopleFirstValue_(row, ["petitioners",
              "petitionerName",
              "petitioner",
              "proposer",
              "ผู้ร้อง",
        ...
        if (name) {
          ...
          parts.length ||(parts = [name]),
          parts.forEach(function(n) {
              var key = n.toLowerCase(); 
              map[key] ||(map[key] = normalizePetitionerRow_({
                    petId: "AUTO-" +(Object.keys(map).length + 1),
                    name: n,
                    phone: _peopleFirstValue_(row, ["petitionerPhone",
                        "phone",
                        "tel",
                        "เบอร์โทรศัพท์"]) || "",
                    caseId: row.caseId || "",
                    remark: "derived-from-maindata"
                  }))
            })
        }
      }),
...
function _Domain_savePetitioner(p) {
  return domainWrite_("savePetitioner", p, function(input) {
      ...
      var repo = _peopleRepo_("Petitioners", "petId"),
      id = String(input.petId || input.id || "").trim() || "P-" + Date.now(),
      normalized = normalizePetitionerRow_(Object.assign({
          }, input || {
          }, {
            petId: id
          })),
      patch = {
        ...
        phone: _appPhoneForSheet_(normalized.phone),
        ...
        petitionerId: normalized.petitionerId,
...
function _Domain_getPetitionerPrintPayload(petId) {
  try {
    ...
    return row ? ok_({
        profileName: "petitioner-detail",
        meta: {
          title: "คณะกรรมาธิการการป้องกันและปราบปรามการทุจริตประพฤติมิชอบ",
          subtitle: "สภาผู้แทนราษฎร",
          classification: "ข้อมูลผู้ร้องและรายละเอียดเรื่องร้องเรียน",
          caseNo: String(row.caseId || ""),
          referenceNo: String(row.petId || "")
        },
        record: row,
        sections: [{
            key: "petitioner",
            title: "ข้อมูลผู้ร้อง",
            fields: {
              "ชื่อผู้ร้อง": row.name,
              "โทรศัพท์": row.phone,
              "ที่อยู่": row.address,
              "เลขประจำตัวผู้ร้อง": row.petitionerId,
              "เลขบัตรประชาชน": row.idCard
            }
          },
          {
            key: "case",
            ...
            fields: {
              "เลขเรื่อง": row.caseId,
              "หมายเหตุ": row.remark,
              "ปรับปรุงล่าสุด": row.updatedAt,
              "สร้างข้อมูลเมื่อ": row.createdAt
            }
          }
        ]
...
function apiDeletePetitioner(payload) {
  return writeGateway_("apiDeletePetitioner", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_deletePetitioner(_payloadValue_(input, ["petId",
              "id"])), "ลบข้อมูลผู้ร้องเรียนสำเร็จ", "ลบข้อมูลผู้ร้องเรียนไม่สำเร็จ"); 
      if (res && res.ok)try {
...
function _peoplePersonnelRowsByKind_(kind, options) {
  ...
  var getter = "ops" ===(kind = String(kind || "staffs")) ? _Domain_getPersonnelOps: "comms" === kind ? _Domain_getPersonnelComms: "subcommitteeMembers" === kind ? _Domain_getPersonnelSubcommittees: _Domain_getPersonnelStaffs,
  cacheStampKey = "ops" === kind ? "personnel_op": "comms" === kind ? "personnel_comm": "subcommitteeMembers" === kind ? "personnel_subcommittees": "personnel_staff",
  cacheStamp,
  cacheKey = "people:personnel-cache-current:" + kind + ":" +(_appIsFnName_("_entityCacheStamp_") ? _entityCacheStamp_(cacheStampKey): "1");
  try {
    ...
    if (_appIsFnName_("_AppCacheGetJson_") && !bypassCache) {
      ...
      if (Array.isArray(hit) &&("subcommitteeMembers" !== kind || 0 !== hit.length))return hit._loadOk = !0,
      hit
    }
  } 
  catch (_cacheReadErr) {
  ...
  try {
    ...
    loadError = String(err && err.message || err || "โหลดข้อมูลบุคลากรไม่สำเร็จ"),
    _appIsFnName_("_recordWarning_") && _recordWarning_("people.rows." + kind, err)
  }
  (rows =(Array.isArray(rows) ? rows: []).map(_peopleNormalizePersonnelContractRow_))._loadOk = !loadError,
  rows._loadError = loadError;
  try {
...
function _peoplePagedPersonnelResponse_(payload, kind, source, message) {
  ...
  total = all.length,
  page = Math.max(1, Number((payload || {
        }).page || 1) || 1),
  reqLimit = Math.max(0, Math.min(Number((payload || {
          }).limit ||(payload || {
          }).pageSize || 0) || 0, 5e3)),
  limit = reqLimit || total,
  rows = reqLimit ? all.slice((page - 1) * limit, page * limit): all,
  loadOk = !1 !== all._loadOk,
  loadError = String(all._loadError || ""),
  out = {
    rows: rows,
    data: rows,
    items: rows,
    ...
    serverPaged: !!reqLimit,
    source: source,
    loadOk: loadOk,
    degraded: !loadOk,
    warning: loadError,
    noFyColumn: !0,
    cacheStamp: "people-personnel-cache-current"
  };
  "ops" === kind &&(out.ops = rows),
  "comms" === kind &&(out.comms = rows, out.personnelComms = rows),
  "staffs" === kind &&(out.staffs = rows, out.personnelStaffs = rows),
  "subcommitteeMembers" === kind &&(out.subcommitteeMembers = rows, out.personnelSubcommittees = rows);
  ...
  ["totalRecords",
    ...
    "loadOk",
    "degraded",
    "warning"].forEach(function(k) {
      response[k] = out[k]
    }),
  "ops" === kind &&(response.ops = rows),
  "comms" === kind &&(response.comms = rows),
  "staffs" === kind &&(response.staffs = rows, response.personnelStaffs = rows),
  "subcommitteeMembers" === kind &&(response.subcommitteeMembers = rows, response.personnelSubcommittees = rows),
  response
}
function apiGetPersonnelComms(payload) {
  var _auth = _peopleSafeReq_(payload, "viewer", "apiGetPersonnelComms");
  return _auth.ok ? _peoplePagedPersonnelResponse_(payload = _auth.payload, "comms", "Personnel_Comm",
    "โหลดข้อมูลบุคลากรคณะกรรมาธิการสำเร็จ"): _auth.result
}
...
function apiGetPersonnelStaffs(payload) {
  ...
  return _auth.ok ? _peoplePagedPersonnelResponse_(payload = _auth.payload, "staffs", "Personnel_Staff",
    "โหลดข้อมูลบุคลากรจาก Personnel_Staff สำเร็จ"): _auth.result
}
function apiGetPersonnelSubcommittees(payload) {
  ...
  if (!0 ===(payload = _auth.payload).forceFresh || !0 === payload.forceRefresh || !0 === payload.noCache || !0 === payload.reload) {
    ...
    return directRows =(Array.isArray(directRows) ? directRows: []).map(_peopleNormalizePersonnelContractRow_),
    ok_({
        rows: directRows,
        data: directRows,
        items: directRows,
        ...
        list: directRows,
        values: directRows,
        subcommitteeMembers: directRows,
        personnelSubcommittees: directRows,
        totalRecords: directRows.length,
        total: directRows.length,
        page: 1,
        limit: directRows.length,
        pageSize: directRows.length,
        totalPages: 1,
        serverPaged: !1,
        source: "Personnel_Subcommittees/direct-forceFresh",
        loadOk: !0
...
function _peopleBundleDate_(value) {
  var raw = _normalizePeopleDateValue_(value || ""),
  m = String(raw || "").match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
  if (!m)return null;
  var y = Number(m[1]);
  y > 2400 &&(y -= 543);
...
function _peopleIsActiveOnDate_(row, date) {
  ...
  start = _peopleBundleDate_(row.startDate || row["วันที่เริ่ม"] || row["วันเริ่ม"]),
  end = _peopleBundleDate_(row.endDate || row["วันที่สิ้นสุด"] || row["วันสิ้นสุด"]);
...
function _peopleNormalizePersonnelContractRow_(row) {
  ...
  status;
  return _peopleIsActiveStatus_(out.status || out["สถานะ"] || "") &&(out.endDate = "", out["วันที่สิ้นสุด"] = "",
    out["วันสิ้นสุด"] = ""),
  out.fiscalYear = out.fiscalYear || out.fy || _peopleInferFiscalYearFromDates_(out.startDate || out["วันที่เริ่ม"] || out["วันเริ่ม"],
    out.endDate || out["วันที่สิ้นสุด"] || out["วันสิ้นสุด"]),
  out.contractStamp = out.contractStamp || "people-contract-dto-current",
  _peopleCanonicalPersonnelDto_(out)
}
function _peopleBundleFiscalYears_(bundle) {
  ...
  ["comms",
    "staffs",
    "ops",
    "subcommitteeMembers"].forEach(function(kind) {
      (Array.isArray(bundle && bundle[kind]) ? bundle[kind]: []).forEach(function(row) {
          addFiscalYear_((row = row || {
              }).fy || row.fiscalYear || row["ปีงบประมาณ"]),
          addFiscalYear_(_peopleInferFiscalYearFromDates_(row.startDate || row["วันที่เริ่ม"] || row["วันเริ่ม"],
              row.endDate || row["วันที่สิ้นสุด"] || row["วันสิ้นสุด"]))
        })
    }),
...
function _peopleBundleSummary_(bundle, selectedFy) {
  ...
  var range = _peopleFiscalYearRange_(selectedFy || ""),
  ...
  return(Array.isArray(bundle && bundle.staffs) ? bundle.staffs: []).forEach(function(row) {
      ...
      _peopleDateOverlapsRange_(_peopleBundleDate_(row.startDate || row["วันที่เริ่ม"] || row["วันเริ่ม"]),
        _peopleBundleDate_(row.endDate || row["วันที่สิ้นสุด"] || row["วันสิ้นสุด"]), range) &&(monthlyRateTotal += amount,
        _peopleIsActiveOnDate_(row, today) &&(activeStaffs++, currentMonthlyExpense += amount))
    }),
  {
    comms: (bundle.comms || []).length,
    staffs: (bundle.staffs || []).length,
    ops: (bundle.ops || []).length,
    activeStaffs: activeStaffs,
    selectedFiscalYear: String(selectedFy || ""),
    monthlyRateTotal: monthlyRateTotal,
    currentMonthlyExpense: currentMonthlyExpense,
    contractStamp: "people-performance-split-current",
    generatedAt: (new Date).toISOString()
  }
}
function _peoplePageBundleCacheKey_(selectedFy) {
  var cacheStamps = ["personnel_comm",
    "personnel_staff",
    "personnel_op",
    "personnel_subcommittees",
    "salarysettings"].map(function(k) {
      try {
        return k + ":" +(_appIsFnName_("_entityCacheStamp_") ? _entityCacheStamp_(k): "1")
      } 
      catch (_e) {
...
function _apiGetPeoplePageBundleCore_(payload) {
  ...
  var requestedFy = String(payload.fy || payload.fiscalYear || payload.year || payload.budgetFy || payload.selectedFiscalYear || payload.currentFy || payload["ปีงบประมาณ"] || "").replace(/[^0-9]/g,
    ""),
  cacheKey = _peoplePageBundleCacheKey_(requestedFy || "latest");
  if (!0 !== payload.forceFresh && _appIsFnName_("_AppCacheGetJson_")) {
    var cached = _AppCacheGetJson_(cacheKey);
    if (cached && cached.loadOk)return cached.cached = !0,
    cached.cacheStatus = "hit",
    ok_(cached, "โหลดข้อมูลหน้าบุคลากรสำเร็จ")
  }
  try {
    var bundle = _Domain_getPersonnelDirectoryBundle(),
    selectedFy = requestedFy,
    salarySettings = {
    };
    try {
    ...
    if (selectedFy && fiscalYears.indexOf(selectedFy) < 0 && fiscalYears.unshift(selectedFy), selectedFy = selectedFy || fiscalYears[0] || "",
      !requestedFy && "function" == typeof getSalarySettings)try {
    ...
    var summary = _peopleBundleSummary_(bundle, selectedFy),
    data = {
      comms: bundle.comms || [],
      staffs: bundle.staffs || [],
      ops: bundle.ops || [],
      subcommitteeMembers: bundle.subcommitteeMembers || [],
      personnelSubcommittees: bundle.subcommitteeMembers || [],
      ...
      directoryPerformance: !0,
      performance: {
        directoryReadModel: !0,
        directoryCacheStatus: String(bundle.cacheStatus || "unknown"),
        selectedFiscalYear: selectedFy
      }
    };
    data.committeeMembers = data.comms,
    data.staffMembers = data.staffs,
    data.operationStaff = data.ops;
    ...
    var salaryPayments = includeSalaryRows ? _peopleSalaryPaymentsRows_(): [],
    salaryByName = {
    };
    ...
    function _peopleSalarySettingRateCurrent_(role) {
      ...
      for (var keys = "advisor" === role ? ["adv",
          "advisor",
          "ที่ปรึกษา",
          "advisorRate",
          "advisorSalary"]: "academic" === role ? ["aca",
          "academic",
          "นักวิชาการ",
          "academicRate",
          "academicSalary"]: "secretary" === role ? ["sec",
          "secretary",
          "เลขานุการ",
          "secretaryRate",
          "secretarySalary"]: ["exp",
    ...
    data.staffs =(data.staffs || []).map(function(row) {
        ...
        var k = String(row.name || row.fullName || row["ชื่อ-สกุล"] || "").trim().toLowerCase(),
        pay = k && salaryByName[k],
        amount = Number(String(row.salaryAmount || row.salary || row.monthlySalary || row.monthlyRate || row.payAmount || row.salaryRate || row["จำนวนเงิน"] || row["เงินเดือน"] || 0).replace(/,/g,
            "")); 
        return(!isFinite(amount) || amount <= 0) && pay &&(row.salaryAmount = pay.salaryAmount, row.salary = pay.salaryAmount,
          row.monthlySalary = pay.salaryAmount, row.monthlyRate = pay.monthlyRate || pay.salaryAmount),
        row
      }),
    ...
    data.salaryRows =(data.staffs || []).filter(function(row) {
        ...
        amount = Number(String(row.salaryAmount || row.salary || row.monthlySalary || row.monthlyRate || row.payAmount || row.salaryRate || row["จำนวนเงิน"] || row["เงินเดือน"] || 0).replace(/,/g,
            "")); 
        return(!isFinite(amount) || amount <= 0) && salaryByNameCurrent[k] &&(amount = salaryByNameCurrent[k]),
        (!isFinite(amount) || amount <= 0) &&(amount = _peopleSalarySettingRateCurrent_(_peopleSalaryRoleKeyCurrent_(row))),
        row.salaryAmount = isFinite(amount) ? amount: 0,
        row.salary = row.salaryAmount,
        row.monthlySalary = row.salaryAmount,
        row.monthlyRate = row.salaryAmount,
        row
      }).filter(function(row) {
        return Number(row.salaryAmount || 0) > 0
      }),
    includeSalaryRows ||(data.salaryRows = [], data.salaryPayments = [], data.salaryRowsDeferred = !0),
    data.historyRows = data.historyRows || [],
    data.peopleProductionDto = !0,
    data.peopleDtoOwner = "Code_33_Domain_People.apiGetPeoplePageBundle",
    data.contractStamp = "people-page-bundle-dto-ppl-current";
    try {
      _appIsFnName_("_AppCachePutJson_") && _AppCachePutJson_(cacheKey, data, Math.max(120, Math.min(Number(payload.cacheTtlSeconds || 240) || 240,
            600)))
    } 
    ...
    ok_({
        ...
        ops: [],
        subcommitteeMembers: [],
        personnelSubcommittees: [],
        fiscalYears: requestedFy ? [requestedFy]: [],
        selectedFiscalYear: requestedFy || "",
        salarySettings: {
        },
        summary: {
          currentMonthlyExpense: 0
        ...
        totalRecords: 0,
        page: 1,
        limit: Math.max(25, Number(payload.limit || payload.pageSize || 25) || 25),
        warning: String(e && e.message || e || "โหลดข้อมูลบุคลากรไม่สำเร็จ")
      }, "โหลดหน้าบุคลากรแบบจำกัด")
  }
}
"function" == typeof _Domain_getPersonnelComms &&(AppDomain.getPersonnelComms = _Domain_getPersonnelComms),
"function" == typeof _Domain_getPersonnelOps &&(AppDomain.getPersonnelOps = _Domain_getPersonnelOps),
"function" == typeof _Domain_getPersonnelStaffs &&(AppDomain.getPersonnelStaffs = _Domain_getPersonnelStaffs),
"function" == typeof _Domain_getPersonnelDirectoryBundle &&(AppDomain.getPersonnelDirectoryBundle = _Domain_getPersonnelDirectoryBundle),
"function" == typeof _Domain_savePersonnelComm &&(AppDomain.savePersonnelComm = _Domain_savePersonnelComm),
"function" == typeof _Domain_savePersonnelOp &&(AppDomain.savePersonnelOp = _Domain_savePersonnelOp),
"function" == typeof _Domain_savePersonnelStaff &&(AppDomain.savePersonnelStaff = _Domain_savePersonnelStaff),
"function" == typeof _Domain_deletePersonnelComm &&(AppDomain.deletePersonnelComm = _Domain_deletePersonnelComm),
"function" == typeof _Domain_deletePersonnelOp &&(AppDomain.deletePersonnelOp = _Domain_deletePersonnelOp),
"function" == typeof _Domain_deletePersonnelStaff &&(AppDomain.deletePersonnelStaff = _Domain_deletePersonnelStaff),
"function" == typeof _Domain_getPersonnelSubcommittees &&(AppDomain.getPersonnelSubcommittees = _Domain_getPersonnelSubcommittees),
"function" == typeof _Domain_savePersonnelSubcommittee &&(AppDomain.savePersonnelSubcommittee = _Domain_savePersonnelSubcommittee),
"function" == typeof _Domain_deletePersonnelSubcommittee &&(AppDomain.deletePersonnelSubcommittee = _Domain_deletePersonnelSubcommittee),
"function" == typeof _Domain_getSubcommitteeList &&(AppDomain.getSubcommitteeList = _Domain_getSubcommitteeList),
"function" == typeof _Domain_saveSubcommittee &&(AppDomain.saveSubcommittee = _Domain_saveSubcommittee),
"function" == typeof _Domain_deleteSubcommittee &&(AppDomain.deleteSubcommittee = _Domain_deleteSubcommittee),
"function" == typeof _Domain_getPetitioners &&(AppDomain.getPetitioners = _Domain_getPetitioners), "function" == typeof _Domain_savePetitioner &&(AppDomain.savePetitioner = _Domain_savePetitioner),
"function" == typeof _Domain_getPetitionerPrintPayload &&(AppDomain.getPetitionerPrintPayload = _Domain_getPetitionerPrintPayload);
PeopleDomain.VERSION = "people-domain-final-current", PeopleDomain.normalizeBundle = function(res) {
  ...
  if (data && "object" == typeof data) {
    ...
    !Array.isArray(data.rows) && rows.length &&(data.rows = rows),
    data.totalRecords = Number(data.totalRecords || data.total || rows.length || 0) || 0,
    data.owner = "PeopleDomain.getPageBundle",
    data.domainOwner = "PeopleDomain"
  }
  return res
...
PetitionerDomain.OWNER_CONTRACT_PHASE5 = Object.assign({
  }, PeopleDomain.OWNER_CONTRACT_PHASE5, {
    owner: "PetitionerDomain",
    sourceOfTruth: ["Petitioners",
      "MainData-related-counts"],
...
function _peopleInvalidatePersonnelDomains_(reason) {
  ...
  try {
    _peopleMarkDirtyDomains_(["people",
        "dashboard"], reason || "people-write")
  } 
  catch (_dirtyErr) {
    _appIsFnName_("_recordWarning_") ? _recordWarning_("ec", _dirtyErr): _appIsFnName_("_logWarn_") ? _logWarn_("ec",
      {
...
function apiDeletePersonnelComm(payload) {
  return writeGateway_("apiDeletePersonnelComm", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_deletePersonnelComm(_payloadValue_(input, ["id",
              "personnelId",
              "commId"])), "ลบข้อมูลคณะกรรมาธิการสำเร็จ", "ลบข้อมูลคณะกรรมาธิการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiDeletePersonnelComm"),
      res
...
function apiDeletePersonnelOp(payload) {
  return writeGateway_("apiDeletePersonnelOp", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_deletePersonnelOp(_payloadValue_(input, ["id",
              "personnelId",
              "opId"])), "ลบข้อมูลเจ้าหน้าที่ฝ่ายเลขานุการสำเร็จ", "ลบข้อมูลเจ้าหน้าที่ฝ่ายเลขานุการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiDeletePersonnelOp"),
      res
...
function apiDeletePersonnelStaff(payload) {
  return writeGateway_("apiDeletePersonnelStaff", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_deletePersonnelStaff(_payloadValue_(input, ["id",
              "personnelId",
              "staffId"])), "ลบข้อมูลบุคลากรสำเร็จ", "ลบข้อมูลบุคลากรไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiDeletePersonnelStaff"),
      res
...
function apiDeletePersonnelSubcommittee(payload) {
  return writeGateway_("apiDeletePersonnelSubcommittee", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_deletePersonnelSubcommittee(_payloadValue_(input, ["id",
              "personnelId",
              "subcommitteeId"])), "ลบข้อมูลคณะอนุกรรมาธิการสำเร็จ", "ลบข้อมูลคณะอนุกรรมาธิการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiDeletePersonnelSubcommittee"),
      res
...
function apiSavePersonnelComm(payload) {
  return writeGateway_("apiSavePersonnelComm", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_savePersonnelComm(input), "บันทึกข้อมูลคณะกรรมาธิการสำเร็จ",
        "บันทึกข้อมูลคณะกรรมาธิการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiSavePersonnelComm"),
      res
...
function apiSavePersonnelSubcommittee(payload) {
  return writeGateway_("apiSavePersonnelSubcommittee", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_savePersonnelSubcommittee(input), "บันทึกข้อมูลคณะอนุกรรมาธิการสำเร็จ",
        "บันทึกข้อมูลคณะอนุกรรมาธิการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiSavePersonnelSubcommittee"),
      res
...
function apiSavePersonnelOp(payload) {
  return writeGateway_("apiSavePersonnelOp", payload || {
      ...
      if (!_auth.ok)return _auth.result; 
      input = _auth.payload; 
      var res = normalizeResult_(_Domain_savePersonnelOp(input), "บันทึกข้อมูลเจ้าหน้าที่ฝ่ายเลขานุการสำเร็จ",
        "บันทึกข้อมูลเจ้าหน้าที่ฝ่ายเลขานุการไม่สำเร็จ"); 
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiSavePersonnelOp"),
      res
...
function apiSavePersonnelStaff(payload) {
  return writeGateway_("apiSavePersonnelStaff", payload || {
      ...
      return res && res.ok && _peopleInvalidatePersonnelDomains_("apiSavePersonnelStaff"),
      res
...
function _petitionerRelatedMainDataRows_() {
  var rows = [];
  try {
    _appIsFnName_("_caseProjectedRows_") &&(rows = _caseProjectedRows_(["caseId",
          "id",
          "caseNum",
          "recNo",
          "receiveNo",
...
function _petitionerRelatedCaseDto_(row) {
  ...
  {
    caseId: String(row.caseId || row.id || "").trim(),
    caseNum: String(row.caseNum || row.caseNo || "").trim(),
    recNo: String(row.recNo || row.receiveNo || row["เลขรับ"] || row["เลขรับเรื่อง"] || "").trim(),
    title: String(row.title || row.caseTitle || row.subject || row["เรื่อง"] || row["เรื่องพิจารณา"] || "").trim(),
    caseTitle: String(row.caseTitle || row.title || row.subject || "").trim(),
    petitioners: String(row.petitioners || row.petitioner || row.petitionerName || row["ผู้ร้อง"] || row["ผู้ร้องเรียน"] || "").trim(),
    status: String(row.status || row.caseStatus || row["สถานะ"] || row["สถานะเรื่อง"] || "").trim(),
    recDate: row.recDate || row.receiveDate || row.dateReceived || row["วันที่รับเรื่อง"] || "",
    receiveDate: row.receiveDate || row.recDate || row.dateReceived || "",
...
function apiGetPetitionerRelatedCounts(payload) {
  ...
  try {
    ...
    return ok_({
        counts: counts,
        rowsByName: rowsByName,
        rows: rowsByName[firstName] || [],
        total: counts[firstName] || 0,
        source: "maindata-related-current"
      }, "โหลดจำนวนเรื่องที่เกี่ยวข้องสำเร็จ")
  } 
  catch (e) {
    return ok_({
...
function _peoplePetitionerNormalize_(row, idx) {
  ...
  return Object.assign({
      ...
      petId: _peoplePetitionerText_(_peoplePetitionerPick_(row, ["petId",
            "id",
            "uid",
            "rowId",
            "เลขที่"]) || "PET-" +(idx + 1)),
      name: _peoplePetitionerText_(name),
      phone: _appPhoneForDisplay_(_peoplePetitionerPick_(row, ["phone",
            "tel",
            "mobile",
            "petitionerPhone",
            "เบอร์โทรศัพท์"])),
      address: _peoplePetitionerText_(_peoplePetitionerPick_(row, ["address",
            "fullAddress",
            "ที่อยู่"])),
...
function _peoplePetitionerRowsFromMainData_() {
  ...
  try {
    ...
    _appIsFnName_("_peopleDataServiceRows_") ? cases = _peopleDataServiceRows_("MainData", [], {
        includeDeleted: !1,
        ttl: 120,
        owner: "people.petitionerMainData"
    ...
    (Array.isArray(cases) ? cases: []).forEach(function(row) {
        var names = _peoplePetitionerSplit_(_peoplePetitionerPick_(row = row && "object" == typeof row ? row: {
              ...
              "ผู้ร้อง"])); 
        if (names.length) {
          var phone = _appPhoneForDisplay_(_peoplePetitionerPick_(row, ["petitionerPhone",
                "phone",
                "tel",
                "mobile",
                "เบอร์โทรศัพท์"])); 
          names.forEach(function(name) {
              ...
              map[key] ||(map[key] = {
                  petId: "PET-" +(Object.keys(map).length + 1),
                  name: name,
                  phone: phone,
                  address: "",
                  relatedCount: 0
...
function _petitionerGetListDomainOwnerPhase5_(payload) {
  ...
  var data = {
    ...
    page: 1,
    limit: rows.length,
    source: "apiGetPetitioners-current",
    loadOk: !0,
    degraded: !1
  },
  res = "function" == typeof ok_ ? ok_(data, "โหลดข้อมูลผู้ร้องเรียนสำเร็จ"): {
    ok: !0,
    data: data,
    message: "โหลดข้อมูลผู้ร้องเรียนสำเร็จ"
  };
...
AdminDomain.VERSION = "admin-domain-final-current", AdminDomain.normalizeUserRows = function(result) {
  var data = result && result.data && "object" == typeof result.data ? result.data: result,
  rows;
  return(Array.isArray(data) ? data: Array.isArray(data && data.rows) ? data.rows: Array.isArray(data && data.data) ? data.data: Array.isArray(data && data.items) ? data.items: []).map(function(row,
      idx) {
      return row = row || {
      },
      {
        username: String(row.username || row.user || row.email || row["ชื่อผู้ใช้"] || "").trim(),
        name: String(row.name || row.fullName || row.displayName || row["ชื่อ-สกุล"] || row["ชื่อ"] || "").trim(),
        email: String(row.email || row.mail || row["อีเมล"] || "").trim(),
        role: String(row.role || row.userRole || row["ระดับสิทธิ์"] ||(0 === idx ? "Admin": "Viewer")).trim() || "Viewer",
        active: String(row.active || row.isActive || row.status || row["สถานะ"] || "Y").trim() || "Y"
      }
    }).filter(function(row) {
  ...
  try {
    ...
    return!rows.length && "undefined" != typeof AppRepository && AppRepository.readObjects &&(rows = AdminDomain.normalizeUserRows(AppRepository.readObjects("Users"))),
    AdminDomain.rowsEnvelope(rows, "AdminDomain.listUsers", "โหลดรายชื่อผู้ใช้งานสำเร็จ")
  } 
  catch (e) {
  ...
  try {
    return _appIsFnName_("_routerReq_") &&(payload = _routerReq_(payload, "admin")),
    "function" == typeof auditEvent_ && auditEvent_("admin", {
        route: "apiAdminSaveUser",
        action: String(payload.action || "save")
      }),
    "function" == typeof _saveUserCanonical_ ? normalizeResult_(_saveUserCanonical_(payload), "บันทึกผู้ใช้งานสำเร็จ",
      "บันทึกผู้ใช้งานไม่สำเร็จ"): err_("ADMIN_USER_SAVE_OWNER_UNAVAILABLE")
  } 
  catch (e) {
    return err_(e && e.message ? e.message: String(e))
  }
  ...
  try {
    return _appIsFnName_("_routerReq_") &&(payload = _routerReq_(payload, "admin")),
    "function" == typeof auditEvent_ && auditEvent_("admin", {
        route: "apiAdminDeleteUser",
        username: payload.username || payload.oldUsername || ""
      }),
    "function" == typeof _deactivateUserCanonical_ ? normalizeResult_(_deactivateUserCanonical_(payload),
      "ลบ/ระงับผู้ใช้งานสำเร็จ", "ลบ/ระงับผู้ใช้งานไม่สำเร็จ"): err_("ADMIN_USER_DELETE_OWNER_UNAVAILABLE")
  } 
  catch (e) {
    return err_(e && e.message ? e.message: String(e))
  }
  ...
  "function" == typeof _Domain_saveSubcommittee ? normalizeResult_(_Domain_saveSubcommittee(payload),
    "บันทึกคณะอนุกรรมาธิการสำเร็จ", "บันทึกคณะอนุกรรมาธิการไม่สำเร็จ"): err_("ADMIN_SUBCOMMITTEE_SAVE_OWNER_UNAVAILABLE")
}, AdminDomain.deleteSubcommittee = function(payload) {
  payload = payload || {
  };
  var id = _appIsFnName_("_payloadValue_") ? _payloadValue_(payload, ["id",
      "subcommitteeId"]): payload.id || payload.subcommitteeId;
  return "function" == typeof _Domain_deleteSubcommittee ? normalizeResult_(_Domain_deleteSubcommittee(id),
    "ลบคณะอนุกรรมาธิการสำเร็จ", "ลบคณะอนุกรรมาธิการไม่สำเร็จ"): err_("ADMIN_SUBCOMMITTEE_DELETE_OWNER_UNAVAILABLE")
};
